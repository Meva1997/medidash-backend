...
- `consultations.py` — two routers: `patient_router` (no prefix) handles `POST /patients/{patient_id}/consultations` and `GET /patients/{patient_id}/consultations`; `router` (prefix `/consultations`) handles `GET /consultations/{id}`, add/update diagnoses (doctor only) with version history, `POST /consultations/{consultation_id}/treatments` (409 if one already active), `PATCH /consultations/{consultation_id}/treatments/{treatment_id}` (400 if identical), and `GET /consultations/{consultation_id}/treatments` ordered by active-first then newest. The `/consultations` prefix prevents the wildcard `{consultation_id}` route from shadowing `/health`.
